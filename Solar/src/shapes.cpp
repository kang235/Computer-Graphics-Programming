...
void ShapesC::AddVertex(vector <GLfloat> *a, const glm::vec3 *v)
{
	a->push_back(v->x);
	a->push_back(v->y);
	a->push_back(v->z);
...
{
  modelviewParameter=uniform;
}

void ShapesC::SetColor(GLfloat r, GLfloat g, GLfloat b, GLfloat a)
{
	color[0]=r;
	color[1]=g;
	color[2]=b;
...
void ShapesC::SetColorParamToShader(GLuint uniform)
{
	colorParameter = uniform;
}
